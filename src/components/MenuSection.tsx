...
const MenuSection: React.FC = () => {
  ...
  return (
    ...
    {saladSizesLine}
      ...
      {tabSections.map(({ section, items }) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          activeCategory === 'Beverages' ? (
            ...
            <div key={item.name} className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col relative">
            <img
              src={item.image}
              alt={item.name}
      ...
      {/* Wings Flavors Popover (per item) */}
      {showWingsFlavors.open && showWingsFlavors.itemName && (
        <div className="fixed z-50 bg-white rounded-lg shadow-2xl p-4 w-64 border border-orange-400" 
        style={{
          top: '50%',
          left: '50%',
          transform: 'translate(-50%, -50%)'
        }}>
        <button className="absolute top-2 right-2 text-gray-400 hover:text-gray-700 text-xl" onClick={() => setShowWingsFlavors({ open: false, itemName: null })}>&times;</button>
        <h3 className="text-lg font-bold mb-3 text-gray-900 pr-6">{showWingsFlavors.itemName} Flavors</h3>
        <ul className="list-disc pl-5 text-gray-800 text-sm max-h-64 overflow-y-auto">
